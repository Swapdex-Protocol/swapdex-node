...
pub mod pallet {
    ...
    pub type Claims<T: Config> =
        StorageMap<_, Blake2_128Concat, EthereumAddress, BalanceOf<T>, ValueQuery>;
    ...
    impl<T: Config> GenesisBuild<T> for GenesisConfig<T> {
        fn build(&self) {
            self.claims
                .iter()
                .for_each(|(eth_address, initial_balance)| {
    ...
    impl<T: Config> Pallet<T> {
        /// Claim coins by providing signed message with Ethereum address.
...
impl<T: Config> Pallet<T> {
    ...
    fn process_claim(
        ...
        let imbalance =
            ...
            imbalance.peek()
                != <T::Currency as Currency<T::AccountId>>::PositiveImbalance::zero().peek(),
