...
// Copyright (C) 2020-2021 SwapDEX Network.
...
fn expected_regex() -> Regex {
    Regex::new(
        r"^starkley-chain (\d+\.\d+\.\d+(?:-.+?)?)-([a-f\d]+|unknown)-(.+?)-(.+?)(?:-(.+))?$",
    )
    .unwrap()
}
...
fn version_is_full() {
    ...
    let output = Command::new(cargo_bin("starkley-chain"))
        .args(&["--version"])
        .output()
        .unwrap();
...
fn test_regex_matches_properly() {
    ...
    let captures = expected
        .captures("starkley-chain 2.0.0-da487d19d-x86_64-linux-gnu")
        .unwrap();
    ...
    let captures = expected
        .captures("starkley-chain 2.0.0-alpha.5-da487d19d-x86_64-linux-gnu")
        .unwrap();
    ...
    let captures = expected
        .captures("starkley-chain 2.0.0-alpha.5-da487d19d-x86_64-linux")
        .unwrap();
    assert_eq!(&captures[1], "2.0.0-alpha.5");
    assert_eq!(&captures[2], "da487d19d");
    assert_eq!(&captures[3], "x86_64");
